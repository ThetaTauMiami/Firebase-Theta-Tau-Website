...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const errorMessage = document.getElementById('lblAccountErrorMessage') as HTMLElement;
    ...
    if (accountForm) {
        accountForm.addEventListener('submit', function(event: Event) {
            ...
            if (!emailRegex.test(email)) {
                errorMessage.textContent = 'Please enter a valid miami email address.';
                return;
            }
    ...
    function submitToAPI(verificationAnswer: string): void {
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                ...
                errorMessage.textContent = xhr.responseJSON?.message || 'An error occurred during account creation.';
            }
        });
    }
});